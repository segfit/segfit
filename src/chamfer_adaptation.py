# NOTE: code taken from https://pytorch3d.readthedocs.io/en/latest/_modules/pytorch3d/loss/chamfer.html#chamfer_distance with slight modifications
...
def _chamfer_distance_single_direction(
    ...
    if custom_norm == "huber":
        delta = 1.0
        nearest_neighbours = y[:, x_nn.idx[0, :, 0], :]
        abs_diff = torch.abs(x - nearest_neighbours)
        loss = torch.where(abs_diff < delta, 0.5 * abs_diff**2, delta * (abs_diff - 0.5 * delta))
        cham_x = 10 * torch.mean(loss, axis=2)
