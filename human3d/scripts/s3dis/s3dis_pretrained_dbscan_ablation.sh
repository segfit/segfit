#!/bin/bash
#SBATCH --job-name=s3dis
#SBATCH --output=../../logs/s3dis_dbscan_ablation_%A-%a.out
...
DBSCAN=(0.5 0.6 0.7 0.8 0.9 1.0 1.1 1.2)

CURR_AREA=5  # set the area number accordingly [1,6]
CURR_DBSCAN=${DBSCAN[SLURM_ARRAY_TASK_ID]}
CURR_TOPK=-1
CURR_QUERY=100
...
general.experiment_name="area${CURR_AREA}_pretrained_eps_${CURR_DBSCAN}_topk_${CURR_TOPK}_q_${CURR_QUERY}_fix2" \
