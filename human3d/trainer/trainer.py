...
class InstanceSegmentation(pl.LightningModule):
    ...
    def save_visualizations2(
        ...
        if "labels" in target_full:
            ...
            for instance_counter, (label, mask, full_id) in enumerate(
                zip(
                    target_full["labels"], target_full["masks"], target_full["full_ids"]
                )
            ):
                if label == 255:
                    continue
            ...
            pcd_output = open3d.geometry.PointCloud()
            pcd_output.points = open3d.utility.Vector3dVector(full_res_coords)
            pcd_output.colors = open3d.utility.Vector3dVector(target_output / 255.0)
            pcd_output.normals = open3d.utility.Vector3dVector(original_normals)
        ...
        for did in range(len(sorted_masks)):
            for i in range(sorted_masks[did]["human"].shape[1]):
                ...
                if (part_semseg > 0).sum() > 0:
                    #pdb.set_trace()
                    v.add_points(
                        f"{i}_mhbps_{sort_scores_values[did]['human'][i].item():.2f}",
                        full_res_coords[part_semseg > 0],
                        visible=sort_scores_values[did]["human"][i].item() > 0.01,
                        alpha=1.0,
    ...
    def eval_step(self, batch, batch_idx):
        #print(batch[0].coordinates.min(), batch[0].coordinates.max(), batch[0].coordinates)
        data, target, file_names, clip_feat, clip_pos = batch
        #print(data.coordinates.shape, data.coordinates.min(), data.coordinates.max(), data.coordinates)
        #print(data.features.shape, data.features.min(), data.features.max(), data.features)
        #print(data.original_coordinates[0].shape, data.original_coordinates[0].min(), data.original_coordinates[0].max(), data.original_coordinates)
    ...
    def eval_instance_step(
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            all_human_pred_classes[bid] = self.validation_dataset._remap_model_output(
                all_human_pred_classes[bid].cpu() + label_offset
            )
            all_parts_pred_classes[bid] = self.validation_dataset._remap_model_output(
            ...
            if self.config.general.save_visualizations:
                if "cond_inner" in self.test_dataset.data[idx[bid]]:
                    ...
                    self.save_visualizations(target_full_res[bid],
                                             full_res_coords[bid],
                                             [self.preds[file_names[bid]]['pred_human_masks']],
                                             [self.preds[file_names[bid]]['pred_human_classes']],
                                             file_names[bid],
                                             original_colors[bid],
                                             original_normals[bid],
                                             [self.preds[file_names[bid]]['pred_human_scores']],
                                             sorted_heatmaps=[all_human_heatmaps[bid]],
                                             query_pos=all_query_pos[bid] if len(all_human_query_pos) > 0 else None,
                                             backbone_features=backbone_features,
                                             point_size=self.config.general.visualization_point_size)
                    self.save_visualizations(target_full_res[bid],
                                             full_res_coords[bid],
                                             [self.preds[file_names[bid]]['pred_parts_masks']],
                                             [self.preds[file_names[bid]]['pred_parts_classes']],
                                             "parts " + file_names[bid],
                                             original_colors[bid],
                                             original_normals[bid],
                                             [self.preds[file_names[bid]]['pred_parts_scores']],
                                             sorted_heatmaps=[all_parts_heatmaps[bid]],
                                             query_pos=all_query_pos[bid] if len(all_human_query_pos) > 0 else None,
                                             backbone_features=backbone_features,
                                             point_size=self.config.general.visualization_point_size)"""
    ...
    def save_visualizations3(
        ...
        pred_inst_npy = np.zeros((full_res_coords.shape[0]), dtype=np.int16)
        pred_parts_npy = np.zeros((full_res_coords.shape[0]), dtype=np.int16)
        ...
        for did in range(len(sorted_masks)):
            for i in range(sorted_masks[did]["human"].shape[1]):
                ...
                if sort_scores_values[did]["human"][i] > threshold:
                    pred_human[human_mask] = (np.asarray(self.map2color_instances([i + 1])).T) # CHECK THIS

                parts_mask = sorted_masks[did]["parts"][:,:,i].astype(bool)
                restricted_mask = np.logical_and(human_mask[..., None], parts_mask)

                part_classes = sort_classes[did]["parts"][:,i]
                parts_scores = sort_scores_values[did]["parts"][:,i]
                ...
                if (part_semseg > 0).sum() > 0:
                    ...
                    if sort_scores_values[did]["human"][i] > threshold:
                        ...
                        pred_inst_npy[part_semseg > 0] = num_instances
                        pred_parts_npy[part_semseg > 0] = part_semseg[part_semseg > 0]
        ...
        v.save(f"{self.config['general']['save_dir']}/visualizations/{file_name}")
        return pred_inst_npy, pred_parts_npy, pred_human, pred_output



    def get_preds_no_viz(
        self,
        sorted_masks,
        sort_classes,
        sort_scores_values,
        ...
        num_points = sorted_masks['parts'].shape[0]
        pred_inst_npy = np.zeros((num_points), dtype=np.uint8)
        pred_parts_npy = np.zeros((num_points), dtype=np.uint8)
        pred_scores_npy = list()

        num_instances = 0

        for i in range(sorted_masks["human"].shape[1]):
            human_mask = sorted_masks["human"][:, i].astype(bool)
            
            parts_mask = sorted_masks["parts"][:,:,i].astype(bool)
            restricted_mask = np.logical_and(human_mask[..., None], parts_mask)

            part_classes = sort_classes["parts"][:,i]
            parts_scores = sort_scores_values["parts"][:,i]
            ...
            if (part_semseg > 0).sum() > 0:
                if sort_scores_values["human"][i] > threshold:
                    num_instances += 1
                    pred_inst_npy[part_semseg > 0] = num_instances
                    pred_parts_npy[part_semseg > 0] = part_semseg[part_semseg > 0]
                    pred_scores_npy.append(sort_scores_values["human"][i].item())

        return pred_inst_npy, pred_parts_npy, np.asarray(pred_scores_npy)
    ...
    def eval_instance_step_demo(
        ...
        backbone_features=None,
        threshold = 0.90, #0.85
        inst_min_point_threshold = 10000,
        iou_threshold = 1.0
    ):
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            ...
            #self.config.general.filter_out_instances = True
            if self.config.general.filter_out_instances:
                keep_instances =  np.zeros((sorted_human_masks.shape[1],)).astype(bool) #set()
                pairwise_human_overlap = sorted_human_masks.T @ sorted_human_masks
                normalization = pairwise_human_overlap.max(axis=0)
                norm_overlaps = pairwise_human_overlap / normalization

                for instance_id in range(norm_overlaps.shape[0]):
                    # filter out unlikely masks and nearly empty masks
                    # if not(sort_scores_values[instance_id] < 0.3 or sorted_masks[:, instance_id].sum() < 500):
                    if not ((sort_human_scores_values[instance_id]< threshold) or (sorted_human_masks[:, instance_id].sum() < inst_min_point_threshold)):
                        overlap_ids = np.nonzero(norm_overlaps[instance_id, :]> iou_threshold)[0]

                        if len(overlap_ids) == 0:
                            keep_instances[instance_id] = True
                        else:
                            max_score = float('-inf')
                            for tmp_inst, tmp_score in zip(overlap_ids, sort_human_scores_values[overlap_ids]):
                                if tmp_score > max_score:
                                    max_score = tmp_score
                                    max_instance = tmp_inst
                            keep_instances[max_instance] = True
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            ...
            print('orig_scores:', sort_human_scores_values)
            retained_humans = (sort_human_scores_values>threshold)
            point_thr_humans = (all_human_pred_masks[bid].sum(axis=0)>inst_min_point_threshold)
            print("orig_num_points:", all_human_pred_masks[bid].sum(axis=0))
            if (retained_humans*point_thr_humans).sum()>0:
                retained_humans = (retained_humans*point_thr_humans).bool()

            all_human_pred_classes[bid] = all_human_pred_classes[bid][retained_humans]
            all_human_pred_masks[bid] = all_human_pred_masks[bid][:, retained_humans]
            all_human_pred_scores[bid] = all_human_pred_scores[bid][retained_humans]
            all_human_heatmaps[bid] = all_human_heatmaps[bid][:, retained_humans]

            all_parts_pred_classes[bid] = all_parts_pred_classes[bid].reshape((16, 5))[:,retained_humans]
            all_parts_pred_masks[bid] = all_parts_pred_masks[bid].reshape((-1, 16, 5))[:, :,retained_humans]
            all_parts_pred_scores[bid] = all_parts_pred_scores[bid].reshape((16, 5))[:,retained_humans]
            all_parts_heatmaps[bid] = all_parts_heatmaps[bid].reshape((-1, 16, 5))[:, :,retained_humans]

            if self.config.general.eval_inner_core == -1:
                #pdb.set_trace()
                if retained_humans.sum()==0:
                    num_points = sorted_human_masks.shape[0]
                    pred_inst_np = np.zeros((num_points), dtype=np.int16)
                    pred_parts_np = np.zeros((num_points), dtype=np.int16)
                    pred_scores_np = np.zeros((0), dtype=float)

                    return (pred_inst_np, pred_parts_np, pred_scores_np)
                ...
                if self.config.general.save_visualizations:
                    ...
                    print("scores:", all_human_pred_scores[bid])
                else:
                    self.preds[file_names[bid]] = {
                        "pred_human_masks": all_human_pred_masks[bid].astype(bool),
                        "pred_human_scores": all_human_pred_scores[bid]
            ...
            if self.config.general.save_visualizations:
                ...
                if save_pyviz3d:
                    pred_inst_np, pred_parts_np, _, _ = self.save_visualizations3(
                    None,
                    full_res_coords[bid],
                    [
                        {
                    ...
                    pred_inst_np, pred_parts_np, pred_scores_np = self.get_preds_no_viz( #self.save_visualizations3( # pred_inst_viz, pred_parts_viz
                        sorted_masks = {
                                "parts": self.preds[file_names[bid]]["pred_parts_masks"],
                                "human": self.preds[file_names[bid]]["pred_human_masks"],
                            },
                        sort_classes = {
                                "parts": self.preds[file_names[bid]]["pred_parts_classes"],
                                "human": self.preds[file_names[bid]]["pred_human_classes"],
                            },
                        sort_scores_values = {
                                "parts": self.preds[file_names[bid]]["pred_parts_scores"],
                                "human": self.preds[file_names[bid]]["pred_human_scores"],
                            },
                    threshold=threshold
                    )
                    #print('human size:', (pred_inst_np>0).sum())
                    #print('part size:', (pred_parts_np>0).sum())

            print("unique instances:", np.unique(pred_inst_np))
            return (pred_inst_np, pred_parts_np, pred_scores_np) #, pred_inst_viz, pred_parts_viz)
    ...
    def eval_step_demo(self, batch, save_pyviz3d=False, threshold = 0.90, inst_min_point_threshold = 10000, iou_threshold=1.0):
        data, target, file_names, clip_feat, clip_pos = batch
        #print(data.coordinates.shape, data.coordinates.min(), data.coordinates.max(), data.coordinates)
        #print(data.features.shape, data.features.min(), data.features.max(), data.features)
        #print(data.original_coordinates[0].shape, data.original_coordinates[0].min(), data.original_coordinates[0].max(), data.original_coordinates)
        ...
        if len(data.coordinates) == 0:
            assert False, "len(data.coordinates)=0!"
            return 0.0
        ...
        if raw_coordinates.shape[0] == 0:
            assert False, "raw_coordinates.shape[0]=0!"
            return 0.0
        ...
        if self.config.general.save_visualizations and save_pyviz3d:
            backbone_features = output["backbone_features"].F.detach().cpu().numpy()
            from sklearn import decomposition
        ...
        res = self.eval_instance_step_demo(
            output,
            target,
            target_full,
            inverse_maps,
            ...
            raw_coordinates,
            data_idx,
            save_pyviz3d=save_pyviz3d,
            backbone_features=rescaled_pca
            if (self.config.general.save_visualizations and save_pyviz3d)
            else None,
            threshold = threshold,
            inst_min_point_threshold = inst_min_point_threshold,
