                ...
                for (gti,gt) in enumerate(gt_instances):
                    ...
                    for pred in gt['matched_pred']:
                        ...
                        if overlap > overlap_th:
                            ...
                            gt_human_visible_parts = gt_human_visible_parts[gt_human_visible_parts != 0]

                            # count instances which are at least partly detected (> overlap threshold)
                            percentage_correct_parts_denominator = len(gt_human_visible_parts)

                            percentage_correct_parts_nominator = float(np.sum(pred['part_iou'] > overlap_th))
                            if percentage_correct_parts_denominator > 0:
                                single_pcp = percentage_correct_parts_nominator / percentage_correct_parts_denominator
                            else:
                                single_pcp = 0.
...
def assign_instances_for_scan(pred: dict, gt_file: str):
    ...
    for uuid in pred_info:
        ...
        for (gt_num, gt_inst) in enumerate(gt2pred):
            ...
            gt_part_single_human[gt_ids % 1000 != gt_inst['instance_id']] = 0  # other humans to background
            gt_part_single_human = gt_part_single_human // 1000
            hist = fast_hist(gt_part_single_human, pred_instance['body_semseg'], 16)  # 27)
            hist = hist[1:, 1:]
...
488, 540, 562, 570, 572, 581, 609, 748, 776, 1156, 1163, 1164, 1165, 1166, 1167, 1168, 1169, 1170, 1171, 1172, 1173, 1174, 1175, 1176, 1178, 1179, 1180, 1181, 1182, 1183, 1184, 1185, 1186, 1187, 1188, 1189, 1190, 1191))
    ...
    if dataset == "human_part_segmentation":
        assert False
        VALID_CLASS_IDS = np.array(list(range(1, 27)))

        CLASS_LABELS = ["rightHand", "rightUpLeg", "leftArm", "head", "leftEye", "rightEye", "leftLeg",
