...
class HumanInstance(object):
    ...
    def __init__(self, mesh_vert_instances, instance_id):
        ...
        self.instance_id = int(instance_id)
        #self.label_id    = int(self.get_label_id(instance_id))
        self.vert_count = (mesh_vert_instances % 1000 == instance_id).sum()
        self.body_semseg = mesh_vert_instances // 1000
...
def get_instances(ids):
    instances = []
    instance_ids = np.unique(ids % 1000)
    for id in instance_ids:
        if id == 0:
            continue
        instance = {
            'instance_id': int(id),
            'vert_count': (ids % 1000 == id).sum(),
            'body_semseg': ids // 1000
        }
