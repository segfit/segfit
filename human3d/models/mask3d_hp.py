...
class Mask3DHumanParts(nn.Module):
    def __init__(self, config, hidden_dim,
                 num_human_queries, num_parts_per_human_queries,
                 ...
                 query_init,
                 clip_proj_dropout,
                 restricted_cross_attention
                 ):
        super().__init__()

        self.restricted_cross_attention = restricted_cross_attention
        self.num_human_queries = num_human_queries
        self.num_parts_per_human_queries = num_parts_per_human_queries

        self.num_queries = self.num_human_queries * (1 + self.num_parts_per_human_queries)
        ...
        self.clip_proj_dropout = clip_proj_dropout

        if self.query_init == "clip_init":
            # 768 is the clip feature dim
            self.clip_proj = nn.Linear(768, hidden_dim, bias=False)
            if self.clip_proj_dropout is not None:
                self.clip_proj_drop = nn.Dropout(self.clip_proj_dropout)
        ...
        self.part_class_embed_head = nn.Linear(hidden_dim, self.num_classes)
        self.human_class_embed_head = nn.Linear(hidden_dim, 2)
    ...
    def forward(self, x, point2segment=None, raw_coordinates=None, is_eval=False,
        ...
        if self.query_init == "clip_init":
            queries = self.clip_proj(clip_feat)

            if self.clip_proj_dropout is not None:
                queries = self.clip_proj_drop(queries)

            query_pos = self.pos_enc(clip_pos, input_range=[scatter_min(coordinates.F,
                                                                        coordinates.C[:, 0].long(), dim=0)[0],
                                                            scatter_max(coordinates.F,
                                                                        coordinates.C[:, 0].long(), dim=0)[0]])
        ...
        predictions_human_class = []
        predictions_part_class = []
        predictions_mask = []
        ...
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                if self.train_on_segments:
                    output_human_class, output_part_class, outputs_mask, attn_mask = self.mask_module(queries,
                                                          mask_features,
                                                          mask_segments,
                                                          len(aux) - hlevel - 1,
                                                          ret_attn_mask=True,
                ...
                if self.restricted_cross_attention:
                    # if human masks does not find humans in particular area, don't allow body parts there
                    human_mask = batched_attn[:, :, :self.num_human_queries]
                    part_mask = batched_attn[:, :, self.num_human_queries:]
                    human_mask = human_mask.repeat(1, 1, self.num_parts_per_human_queries)
                    part_mask = torch.logical_or(human_mask, part_mask)
                    batched_attn[:, :, self.num_human_queries:] = part_mask
                    # TODO
                    batched_attn.permute((0, 2, 1))[batched_attn.sum(1) == rand_idx[0].shape[0]] = False
                ...
                predictions_human_class.append(output_human_class)
                predictions_part_class.append(output_part_class)
                predictions_mask.append(outputs_mask)

        if self.train_on_segments:
            output_human_class, output_part_class, outputs_mask = self.mask_module(queries,
                                                          mask_features,
                                                          mask_segments,
                                                          0,
                                                          ret_attn_mask=False,
        ...
        return {
            'pred_human_logits': predictions_human_class[-1],
            'pred_part_logits': predictions_part_class[-1],
            'pred_masks': predictions_mask[-1],
            'aux_outputs': self._set_aux_loss(
                predictions_human_class, predictions_part_class, predictions_mask
            ),
    ...
    def mask_module(self, query_feat, mask_features, mask_segments, num_pooling_steps, ret_attn_mask=True,
        ...
        # first queries are "human queries"
        outputs_human_class = self.human_class_embed_head(query_feat[:, :self.num_human_queries, :])
        outputs_part_class = self.part_class_embed_head(query_feat[:, self.num_human_queries:, :])
        ...
        if ret_attn_mask:
            ...
            if point2segment is not None:
                return outputs_human_class, outputs_part_class, output_segments, attn_mask
            else:
                return outputs_human_class, outputs_part_class, outputs_mask.decomposed_features, attn_mask

        if point2segment is not None:
            return outputs_human_class, outputs_part_class, output_segments
        else:
            return outputs_human_class, outputs_part_class, outputs_mask.decomposed_features

    @torch.jit.unused
    def _set_aux_loss(self, outputs_human_class, outputs_part_class, outputs_seg_masks):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [
            {"pred_human_logits": a, "pred_part_logits": b, "pred_masks": c}
            for a, b, c in zip(outputs_human_class[:-1], outputs_part_class[:-1], outputs_seg_masks[:-1])
        ]
