...
class SetCriterionHumanPartsNoTwoStage(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, mask_type, parts):
        ...
        if parts:
            target_classes = torch.full(
                src_logits.shape[:2], self.num_parts - 1, dtype=torch.int64, device=src_logits.device
            )
        else:
            target_classes = torch.full(
        ...
        if parts:
            loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight_parts, ignore_index=253)
        else:
            loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight, ignore_index=253)
        losses = {"loss_ce": loss_ce}
        return losses

    def loss_masks(self, outputs, targets, indices, num_masks, mask_type, parts):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks, mask_type, parts=False):
        loss_map = {
            'labels': self.loss_labels,
            'masks': self.loss_masks
        }
        assert loss in loss_map, f"do you really want to compute {loss} loss?"
        return loss_map[loss](outputs, targets, indices, num_masks, mask_type, parts)
    ...
    def forward(self, outputs, targets, mask_type):
        ...
        human_pred = {
            ...
            'pred_masks': [pred_masks[:, :self.num_human_queries] for pred_masks in outputs_without_aux['pred_masks']]
        }

        human_target = [
            {
                'labels': t['human_labels'],
                'masks': t['human_masks']
            }
            for t in targets
        ]
        ...
        for loss in self.losses:
            l_dict = self.get_loss(loss, human_pred, human_target, indices, num_masks, mask_type)
        ...
        human_parts_targets = []
        human_parts_pred = {
            'pred_logits': outputs_without_aux['pred_part_logits'],
            'pred_masks': [outputs_without_aux['pred_masks'][batch_id][:, self.num_human_queries:]
                            for batch_id in range(len(outputs_without_aux['pred_masks']))]
        }
        '''
        for batch_id in range(len(indices)):
            for match_id in range(len(indices[batch_id][0])):
                part_pred_logits = outputs_without_aux['pred_part_logits'][batch_id,
                                    range(indices[batch_id][0][match_id],
                                          self.num_human_queries * self.num_parts_per_human_queries,
                                          self.num_human_queries),
                                    :]
                part_pred_masks = outputs_without_aux['pred_masks'][batch_id][:,
                                  range(indices[batch_id][0][match_id]+self.num_human_queries,
                                        self.num_human_queries * (1 + self.num_parts_per_human_queries),
                                        self.num_human_queries)]
                ...
                # human_idx = targets[batch_id]['full_ids'] % 1000 == indices[batch_id][1][match_id] + 1
                human_parts_targets.append({
                    'labels': targets[batch_id]['labels'],
                    'masks': targets[batch_id]['masks']
                })

        human_parts_pred['pred_logits'] = torch.stack(human_parts_pred['pred_logits'])
        '''

        part_indices = self.matcher(human_parts_pred, targets, mask_type)

        for loss in self.losses:
            l_dict = self.get_loss(loss, human_parts_pred, targets, part_indices, num_masks, mask_type, parts=True)
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                human_pred = {
                    ...
                    'pred_masks': [pred_masks[:, :self.num_human_queries] for pred_masks in aux_outputs['pred_masks']]
                }

                indices = self.matcher(human_pred, human_target, mask_type)

                for loss in self.losses:
                    l_dict = self.get_loss(loss, human_pred, human_target, indices, num_masks,
                                           mask_type)
                    losses.update({f"human_{k}_{i}": v for k, v in l_dict.items()})
                    #for k, v in l_dict.items():
                    #    if v.isnan().cpu().item():
                    #        print("jkjk")
                ...
                human_parts_targets = []
                human_parts_pred = {
                    'pred_logits': aux_outputs['pred_part_logits'],
                    'pred_masks': [aux_outputs['pred_masks'][batch_id][:, self.num_human_queries:]
                                   for batch_id in range(len(aux_outputs['pred_masks']))]
                }
                '''
                for batch_id in range(len(indices)):
                    for match_id in range(len(indices[batch_id][0])):
                        part_pred_logits = aux_outputs['pred_part_logits'][batch_id,
                                           range(indices[batch_id][0][match_id],
                                                 self.num_human_queries * self.num_parts_per_human_queries,
                                                 self.num_human_queries),
                                           :]
                ...
                for loss in self.losses:
                    l_dict = self.get_loss(loss, human_parts_pred, targets, part_indices, num_masks,
                                           mask_type, parts=True)
                    losses.update({f"parts_{k}_{i}": v for k, v in l_dict.items()})
                    #for k, v in l_dict.items():
                    #    if v.isnan().cpu().item():
                    #        print("jkjk")
